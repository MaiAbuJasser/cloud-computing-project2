...
con=pymysql.connector.connect(host='localhost',username='root',password='12345'
,database='flaskdb')
cur = con.cursor()
cur.execute("CREATE TABLE if not exists images(key INT primary key,img TEXT)")
...
def req():  
  #  global miss, hit, policyy, hitRate, missRate, memcache, totalSize, con, cur
    if request.method == 'POST' :
        try:
            con = MySQLdb.connector.connect(host='localhost', username='root', password='12345', database='flaskdb')
            cur = con.cursor()    
